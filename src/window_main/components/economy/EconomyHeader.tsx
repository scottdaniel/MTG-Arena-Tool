...
function getContextCounts(state: EconomyState) {
  ...
  for (let n = 0; n < state.sortedChanges.length; n++) {
    ...
    if (change.delta.gemsDelta != undefined) {
      ...
      else state.dayList[state.daysago].gemsSpent += Math.abs(change.delta.gemsDelta);
    }
    if (change.delta.goldDelta != undefined) {
      ...
      else state.dayList[state.daysago].goldSpent += Math.abs(change.delta.goldDelta);

      // console.log(economyId, "> ", change.date, " > ", change.delta.goldDelta);
    }

    if (change.delta.cardsAdded) {
      state.dayList[state.daysago].cardsEarned += change.delta.cardsAdded.length;
    }
    if (change.delta.vaultProgressDelta > 0) {
      state.dayList[state.daysago].vaultProgress += change.delta.vaultProgressDelta;
    }
...
export function EconomyHeader(props: EconomyHeaderProps) {
  ...
  const total = pd.economy.boosters.reduce((accumulator: number, booster: { count: number }, ) => (accumulator + booster.count), 0);
  // TODO: remove this any cast once renderer-util is a typescript file.
  const vaultTotal = formatPercent(pd.economy.vault / 100, vaultPercentFormat as any);
